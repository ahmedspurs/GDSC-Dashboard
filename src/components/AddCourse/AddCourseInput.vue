<template>
  <form
    ...
    @submit.prevent="newCetrtificate()"
    ref="form"
  >
...
export default {
  ...
  methods: {
    ...
    async newCetrtificate() {
      ...
      if (await this.$store.dispatch("addCourse", data)) {
        this.responseAlert("تمت إضافة الشهادة الثانوي بنجاح", "تم", "success");
      } else {
        this.responseAlert(
          " حدث خطاء اثناء اضافة الشهادة الثانوي ",
          "عفوا",
          "error"
        );
      }
